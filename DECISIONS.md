...
- **Schema-versionering + migratie:** elke load gaat door `migrate()`, die ontbrekende collecties aanvult en oudere datamodellen omzet (bijv. vaste vriezergerechten uit schema ≤3 worden voorraad-items). Geïmporteerde backups en binnenkomende sync-data gaan door dezelfde normalisatie.
...
**Hoe het werkt:** het kookboek is component-gebaseerd — bereidingen (kip/rund/vis/ei/vega), sauzen, koolhydraten, groenten, kruidenmixen en complete gerechten, vrij te combineren in de kookronde-planner. **Het kookboek begint bewust leeg**: de gebruiker vult alles zelf (expliciete keuze — eerdere seed-recepten zijn op verzoek verwijderd). Het invoerformulier dekt alles: categorie, tijd, porties, macro's per portie, ingrediënten (één per regel, mét hoeveelheid zodat de boodschappen-generator kan optellen), stappen, en voor kruidenmixen batch/houdbaarheid/dosering. Recepten zijn achteraf te bewerken. Een gerecht met macro's is met één tik te loggen als maaltijd.

**Toegepaste best practices:**
- **Referentie voor eigen macro's:** bij het onderzoek is een per-ingrediënt geverifieerde macrotabel opgesteld (USDA FoodData Central + NEVO-gebaseerde bronnen) voor de *bereide* toestand — kipfilet 165 kcal/31 g eiwit per 100 g bereid, gekookte rijst 130 kcal/100 g, enz. Handig als naslaglijst bij het zelf invullen van recept-macro's; zie de PR-historie ("Cookbook with verified nutrition data") voor de volledige tabel. ([USDA FDC](https://fdc.nal.usda.gov/), [Voedingscentrum](https://www.voedingscentrum.nl))
- **Nederlandse gehakt-conventie (voor wie zelf macro's invult):** NEVO's "rul bereid" rekent het bakvet mee in het gerecht (331 kcal/100 g voor 15%-gehakt) waar USDA's "drained" waarde (218 kcal) het vet weggiet — kies de stijl die past bij hoe je kookt.
- **Kruidenmixen:** eigen mixen krijgen batch-, houdbaarheids- en doseringsvelden. Geverifieerde vuistregels voor het invullen: piek-aroma ±6 maanden, kwaliteitsvenster 1–2 jaar (McCormick zegt 1–2 jaar voor blends; specerijenhandel noemt 4–8 maanden piek), nooit "bedorven" in voedselveiligheidszin; dosering ~1 el (≈8 g) per 500 g vlees. ([McCormick](https://www.mccormick.com/blogs/how-to/how-long-do-spices-last), [RawSpiceBar](https://rawspicebar.com/blogs/spices-101/how-long-do-ground-spices-last))
- Vriezerporties loggen automatisch mee in het maaltijdlog (één handeling = voorraad − 1 én macro's geteld).

**Kookronde-planner (vervangt de vaste 4-rondes-rotatie):**
- Je stelt per 16-daagse ronde zelf 4 gerechten samen: een **compleet gerecht** uit je eigen kookboek óf een **eigen mix** van componenten (eiwitbereiding + saus + koolhydraat + groente). Een mix wordt één gerecht met opgetelde macro's, en is later met één tik als één maaltijd te loggen.
- De **boodschappenlijst wordt gegenereerd** uit de ingrediënten van alle geplande gerechten, met échte hoeveelheden-optelling: een parser herkent aantal + eenheid (g/kg, ml/l, el/tl, blik, teen, bol, plak, stengel, losse aantallen en breuken zoals ½ en 1½), normaliseert Nederlandse meervouden (uien→ui bij het optellen, 3× ui → "3 uien" bij het tonen) en telt per (eenheid, ingrediënt) op — "1 ui" + "2 uien" wordt dus "3 uien", 550 g + 600 g wordt "1,15 kg". Regels zonder herkenbare hoeveelheid ("scheut azijn") worden als aantal geteld. Verschillende eenheden van hetzelfde ingrediënt (tenen vs. bol knoflook) blijven bewust gescheiden regels — samenvoegen zou een omrekentabel vergen en fouten introduceren. Afgevinkte regels blijven staan zolang de samengetelde regel niet wijzigt.
...
- Oude data migreert automatisch: vaste gerechten met porties > 0 worden voorraad-items (schema v4; sinds het leegmaken van het kookboek zonder macro's — die vult de gebruiker zelf).
